...
private:
  ...
  void handle_market_order(const std::shared_ptr<market_order> &order_, const candlestick_data_aggregate &candles_,
                           std::list<std::shared_ptr<order_event>>::iterator &it_);
  void handle_buy_market_order(const std::shared_ptr<market_order> &order_, const candlestick_data_aggregate &candles_,
                               std::list<std::shared_ptr<order_event>>::iterator &it_);
  void handle_sell_market_order(const std::shared_ptr<market_order> &order_, const candlestick_data_aggregate &candles_,
                                std::list<std::shared_ptr<order_event>>::iterator &it_);
};
