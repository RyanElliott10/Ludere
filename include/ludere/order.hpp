...
{
    enum class order_lifetime_durations
    {
        HOUR = 3600,
        DAY = 3600 * 24,
        WEEK = 3600 * 24 * 7
    ...
    explicit order_lifetime(order_lifetime_durations type_, time_t baseTime_)
...
struct order
{
    ...
    enum class order_types
    {
        LIMIT,
        MARKET
    };
    enum class order_signals
    {
        BUY,
        SELL
    };

    order(std::string security_, uint32_t num_shares_, order::order_signals order_signal_, order::order_types order_type_,
          order::position_types position_type_, order_lifetime order_lifetime_, strategy_callback_def callback_)
    ...
    order::order_signals m_order_signal;
    order::position_types m_position_type;
    order::order_types m_order_type;
...
{
    limit_order(std::string security_, uint32_t num_shares_, order::order_signals order_signal_,
                order::position_types position_type_, float limit_price_, order_lifetime order_lifetime_,
                strategy_callback_def callback_)
            : order(std::move(security_), num_shares_, order_signal_, order::order_types::LIMIT, position_type_,
...
{
    market_order(std::string security_, uint32_t num_share_, order::order_signals order_signal_,
                 order::position_types position_type_, float market_price_, order_lifetime order_lifetime_,
                 strategy_callback_def callback_)
            : order(std::move(security_), num_share_, order_signal_, order::order_types::MARKET, position_type_,
