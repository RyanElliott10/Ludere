...
void exchange::trade()
{
    ...
    candlestick_data_aggregate candles_;
...
void exchange::stream_data(const candlestick_data_aggregate &data_)
{
...
void exchange::handle_order_event(const std::shared_ptr<event> &event_, const candlestick_data_aggregate &candles_)
{
    std::shared_ptr<order_event> order_event_ = std::dynamic_pointer_cast<order_event>(event_);
    m_order_queue.push_back(order_event_);
}

// TODO: An actual queue is based on price. Best fill prices (for seller) are placed on top, while those at the bid
//      price are placed on the bottom
void exchange::fill_orders(const candlestick_data_aggregate &candles_)
{
    auto it_ = m_order_queue.begin();
    while (it_ != m_order_queue.end()) {
        if ((*it_)->m_order->is_expired(candles_.m_timestamp)) {
        ...
        // TODO: This is a bug with a temporary workaround. If an order isn't filled, the iterator is never incremented.
        //      Control of the iterator for incrementing should be in the hands of this function.
...
void
exchange::handle_limit_order(const std::shared_ptr<limit_order> &order_, const candlestick_data_aggregate &candles_,
